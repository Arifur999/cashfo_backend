...
// In-memory refresh-token blacklist for platform users, keyed by the token's
// `jti` claim. Kept as its own module-private instance (mirrors admin-auth's
// TokenBlacklistService) rather than sharing admin-auth's, so the end-user
// auth domain doesn't depend on an internal admin module.
//
